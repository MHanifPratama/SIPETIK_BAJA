  ...
  <form method="post" action="\updateBus\<?=$bus['id_bus'] ?>">
    <div class="form-row">
      <div class="form-group col-md-6 mb-3">
          ...
          <input type="text" class="form-control" id="nama_bus" name="nama_bus" value="<?=$bus['nama_bus']?>">
      </div>
      <div class="form-group col-md-6 mb-3">
      <select class="form-select text-dark" type="text" name="id_supir" id="id_supir">
            <?php foreach ($supir as $supir) : ?>
                <?php if ($supir['id_supir'] == $bus['id_supir']) : ?>
                    <option selected value="<?= $supir['id_supir'] ?>"><?= $supir['nama_supir'] ?></option>
                <?php else : ?>
      ...
      <select class="form-select text-dark" type="text" name="id_jadwal" id="id_jadwal">
            <?php foreach ($jadwal as $jadwal) : ?>
                <?php if ($jadwal['id_jadwal'] == $bus['id_jadwal']) : ?>
                    <option selected value="<?= $jadwal['id_jadwal'] ?>"><?=  $jadwal['tanggal'] ,"   ", $jadwal['waktu_keberangkatan']?></option>
                <?php else : ?>
      ...
      <select class="form-select text-dark" type="text" name="id_tipe" id="id_tipe">
            <?php foreach ($tipeBus as $tipe) : ?>
                <?php if ($tipe['id_tipe'] == $bus['id_tipe']) : ?>
                    <option selected value="<?= $tipe['id_tipe'] ?>"><?=$tipe['tipe']?></option>
                <?php else : ?>
      ...
      <select class="form-select text-dark" type="text" name="id_perjalanan" id="id_perjalanan">
            <?php foreach ($perjalanan as $prjl) : ?>
                <?php if ($prjl['id_perjalanan'] == $bus['id_perjalanan']) : ?>
                    <option selected value="<?= $prjl['id_perjalanan'] ?>"><?=  $prjl['kota_awal'],"  -  ",$prjl['kota_akhir']?></option>
                <?php else : ?>
